...
convert (FILE *fp, int length, int highValue, int lowValue)
{
	...
	while (i < length - 44)	// Move through each byte in the file. Hardcoded to skip past the header.
	{
		...
		if (c >= highValue)	// High state defaults to any sample above 252.
		{
		...
		else if (c <= lowValue)	// Low state is any sample below 3.
		{
			...
			if (flipState != oldState)	// This is a zero cross-over, write out the time since last crossover (count) and reset it.
			{
				...
				if (count > 0xFF) count = 0xFF;	// This hardcoded truncation will mess with any extremely low frequency sounds by preventing the period from being any higher than 255. This frequency floor will depending on your playback routine rate.
